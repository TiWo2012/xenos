...
void *map_page(void *phys, void *virt, uint64_t flags) {
  ...
  if (!(e & PAGE_PRESENT)) {
    page_table *p =
        (page_table *)pmm::phys_to_virt((uint64_t)pmm::alloc_page());
    if (!p)
      return nullptr;
    memory::memset(p, 0, 4096);
    pml4->entries[i4] =
        pmm::virt_to_phys(p) | PAGE_PRESENT | PAGE_WRITE | (flags & PAGE_USER);
  }
  ...
  if (!(e & PAGE_PRESENT)) {
    ...
    p2->entries[i2] =
        pmm::virt_to_phys(pt) | PAGE_PRESENT | PAGE_WRITE | (flags & PAGE_USER);
  }
...
void handle_page_fault(uint64_t addr, uint64_t error_code) {
  ...
  serial::printf("  P=%d W=%d U=%d R=%d I=%d PK=%d\n", (int)(error_code & 1),
                 ...
                 (int)((error_code >> 3) & 1), (int)((error_code >> 4) & 1),
                 (int)((error_code >> 5) & 1));
